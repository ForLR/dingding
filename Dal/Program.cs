...
public void UpdateData()
{
    using (MySqlDBContext db = new MySqlDBContext())
    {
        ...
        var userList = db.user.Where(x => x.getdata.Value == 1).ToList();

        var updateTime = db.yuanshijilu.Any() ? Convert.ToDateTime(db.yuanshijilu.Max(x => x.考勤时间)).Date : new DateTime(2019, 4, 1);
        ...
        db.qingjia.RemoveRange(db.qingjia.ToList());
        if (db.SaveChanges() > 0) { Console.WriteLine("原始数据更新完成"); };

        var ids = userList.Select(x => x.user_id).Aggregate((x, y) => x + "," + y);
        var endTime = Convert.ToInt64((DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds * 1000);
        var startTime = Convert.ToInt64((DateTime.UtcNow.AddDays(-160) - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds * 1000);
        //更新请假信息
        var status = dd.Getleavestatus(ids, startTime, endTime, 0, 20);
        var leave_status = status.result.leave_status.Count;
        for (int i = 0; i < leave_status; i++)
        {
            var result = leave_status[i];
            var qj = new qingjia
            {
                user_id = result.userid,

                开始时间 = LongDateTimeToDateTimeString(result.start_time),
                结束时间 = LongDateTimeToDateTimeString(result.end_time),
